	...
	if(!$count_arr_accs > 0){
		...
		if($error_saccs == ''){
			...
			if($error == ''){
				...
				//epps anteriores no se buscan, pq ya se encuentran asignados
				if($op_url == 1){			
					$output = array(
						"status"  => TRUE,
						"ACC_NM_ID" => $ACC_NM_ID,
			...
			$output = array( 
					"status"  => FALSE,
					"error_msg" => 'ERROR 8: '.$error_saccs
				);
		}
	}else{
		$output = array( 
			"status"  => FALSE,
			"error_msg" => 'ERROR 9: El usuario ya cuenta con un Ingreso registrado sin marcar Salida.'
		);
	}
}else{
	$output = array( 
			"status"  => FALSE,
			"error_msg" => 'ERROR 10: '.$error_accs
		);
}
